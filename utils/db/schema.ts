...
export const Users =pgTable('Users',
    {
        ...
        email: varchar('emil', {length: 255}).notNull().unique(),
...
export const CollectedWastes = pgTable('collected_waste', {
    ...
    collectedDate: timestamp('collection_date').notNull().defaultNow(),
    status: varchar('status', { length: 255 }).notNull().default('collected'),
    verificationResult: jsonb('verification_result'), // Add verificationResult field
  });
...
export const Notifications = pgTable("notifications", {
    id: serial("id").primaryKey(),
    userId: integer("user_id").notNull().references(() => Users.id),  // Ensure this line exists
