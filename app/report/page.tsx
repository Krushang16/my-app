...
// Add this function to set the searchBox state when the component loads
const onLoad = (ref: google.maps.places.SearchBox) => {
  ...
  // Add a useEffect to handle the error
  useEffect(() => {
    if (loadError) {
      console.error('Google Maps API failed to load:', loadError);
      toast.error('Location search functionality is limited due to API issues.');
    }
  }, [loadError]);
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Remove all Markdown code block syntax and any backticks
      text = text.replace(/```json|```|`/g, "").trim();
      
      // Handle potential JSON extraction from text
      let jsonText = text;
      
      // If the text contains multiple lines and doesn't start with {, try to find the JSON block
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check if user is null before trying to access user.id
    if (!user) {
    ...
    try {
      const report = await createReport(
        user.id,  // Now we're sure user is not null
        newReport.location,
        newReport.type,
        newReport.amount,
        preview || undefined,
  ...
  // Fetch user and recent reports on mount
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (email) {
        ...
        if (!fetchedUser) {
          fetchedUser = await createUser(email, 'Anonymous User');
        }
        // Debug log: Check fetched user
...
{isLoaded ? (
  ...
  searchBox !== null ? (
    ...
    // Fallback to regular input if SearchBox fails to initialize
    <input
      type="text"
      id="location"
      name="location"
      ...
      required
      className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300"
      placeholder="Enter location manually (Places API loading failed)"
    />
  )
) : (
  // Same fallback for when API isn't loaded
  <input
    type="text"
    id="location"
    name="location"
  ...
  />
        ...
        <Button 
          ...
          disabled={isSubmitting || !user} // disable if user is not loaded
        >
          {isSubmitting ? (
            <>
              <Loader className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" />
